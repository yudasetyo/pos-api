...
<div class="container mx-auto px-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
        ...
        <a href="{{ route('product.create') }}" class="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded shadow w-full sm:w-auto">
            + Add New Product
        </a>
    </div>
        ...
        <table class="min-w-full bg-white shadow-md">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    <th class="py-2 sm:py-4 px-2 sm:px-6 text-left text-xs sm:text-sm font-bold text-gray-700">Category</th>
                    <th class="py-2 sm:py-4 px-2 sm:px-6 text-left text-xs sm:text-sm font-bold text-gray-700">Image</th>
                    <th class="py-2 sm:py-4 px-2 sm:px-6 text-left text-xs sm:text-sm font-bold text-gray-700">Description</th>
            ...
            <tbody>
                @forelse ($products as $product)
                    <tr class="hover:bg-gray-50">
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-xs sm:text-sm text-gray-800">{{ $product->productName }}</td>
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-xs sm:text-sm text-gray-800">{{ $product->ProductCategory->productCategoryName }}</td>
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-xs sm:text-sm text-gray-800">
                            <img src="{{ Storage::url($product->productImage) }}" alt="{{ $product->productName }}" class="rounded-2xl object-cover w-[90px] h-[90px]">
                        </td>
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-xs sm:text-sm text-gray-800">{{ $product->productDescription }}</td>
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-xs sm:text-sm text-gray-800">@currency($product->productPrice)</td>
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-center">
                            <div class="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-2">
                                <a href="{{ route('product.edit', $product) }}" class="text-xs sm:text-sm bg-blue-500 hover:bg-blue-600 text-white py-1 sm:py-2 px-2 sm:px-4 rounded-full shadow inline-flex items-center justify-center">
