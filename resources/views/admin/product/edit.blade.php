...
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    ...
    <form action="{{ route('product.update', $product) }}" method="POST" class="p-6" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="space-y-6 sm:space-y-8">
            <div class="relative">
                <input id="productName" name="productName" type="text" value="{{ $product->productName }}" class="block w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-colors duration-300 peer">
            ...
            <div class="relative">
                <select id="product_category_id" name="product_category_id" class="block appearance-none w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-colors duration-300">
                    <option value="{{ $product->ProductCategory->id }}">{{ $product->ProductCategory->productCategoryName }}</option>
            ...
            <div class="relative">
                ...
                <img src="{{ Storage::url($product->productImage) }}" alt="" class="mt-5 rounded-2xl object-cover w-[90px] h-[90px]">
                @error('productImage')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="relative">
                <textarea name="productDescription" id="productDescription" cols="30" rows="10" class="block w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-colors duration-300 peer">{{ $product->productDescription }}</textarea>
            ...
            <div class="relative">
                <input id="productPrice" name="productPrice" type="number" value="{{ $product->productPrice }}" class="block w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-colors duration-300 peer">
