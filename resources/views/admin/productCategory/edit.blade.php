...
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
        <h2 class="text-2xl font-semibold text-gray-800">Edit Product Category</h2>
    </div>
    
    <form action="{{ route('productCategory.update', $productCategory) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        <div class="space-y-6 sm:space-y-8">
            <div class="relative">
                <input id="productCategoryName" name="productCategoryName" type="text" value="{{ $productCategory->productCategoryName }}" class="block w-full px-4 py-3 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-colors duration-300 peer">
