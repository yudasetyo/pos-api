...
<div class="container mx-auto px-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 text-center sm:text-left mb-4 sm:mb-0">Product Category Dashboard</h2>
        <a href="{{ route('productCategory.create') }}" class="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded shadow w-full sm:w-auto">
            ...
            <tbody>
                @forelse ($productCategories as $productCategory)
                    <tr class="hover:bg-gray-50">
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-xs sm:text-sm text-gray-800">{{ $productCategory->productCategoryName }}</td>
                        <td class="py-2 sm:py-4 px-2 sm:px-6 text-center">
                            <div class="flex flex-col sm:flex-row justify-center space-y-2 sm:space-y-0 sm:space-x-2">
                                <a href="{{ route('productCategory.edit', $productCategory) }}" class="text-xs sm:text-sm bg-blue-500 hover:bg-blue-600 text-white py-1 sm:py-2 px-2 sm:px-4 rounded-full shadow inline-flex items-center justify-center">
                                ...
                                <form action="{{ route('productCategory.destroy', $productCategory) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
